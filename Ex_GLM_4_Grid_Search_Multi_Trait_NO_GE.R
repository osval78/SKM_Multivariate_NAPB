...
for (i in seq_along(folds)) {
 fold <- folds[[i]]
  ...
  model <-SKM::generalized_linear_model(
    ...
    alpha =c(0,.2,0.4,0.6,1), ###Elastic_Net
...
write.csv(GY_Summaries$env,file="Summary_Chis_GLM_Grid_Multitrait_GY_NO_GE.csv")
...
write.csv(Height_Summaries$env,file="Summary_Chis_GLM_Grid_Multitrait_Height_NO_GE.csv")
